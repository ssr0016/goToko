...
import (
	...
	"github.com/ssr0016/gotoko/database/seeders"
...
func (server *Server) Initialize(appConfig AppConfig, dbConfig DBConfig) {
	...
	server.iniliazeAppConfig(appConfig)
...
func (server *Server) initializeDB(dbConfig DBConfig) {
	...
	if dbConfig.DBDriver == "mysql" {
		dsn := fmt.Sprintf("%s:%s@tcp(%s:%s)/%s?charset=utf8&parseTime=True&loc=Local", dbConfig.DBUser, dbConfig.DBPassword, dbConfig.DBHost, dbConfig.DBPort, dbConfig.DBName)
		server.DB, err = gorm.Open(mysql.Open(dsn), &gorm.Config{})
		if err != nil {
			panic("Failed to connecting to the database server")
		}
	} else {
...
func (server *Server) iniliazeAppConfig(appConfig AppConfig) {
...
func GetPaginationLinks(config *AppConfig, params PaginationParams) (PaginationLinks, error) {
	...
	return PaginationLinks{
		CurrenPage: fmt.Sprintf("%s/%s?page=%s", config.AppURL, params.Path, fmt.Sprint(params.CurrentPage)),
